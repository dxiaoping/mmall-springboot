...
public class JsonUtil {
    ...
    static{
        /**
         * ALWAYS对象的所有字段全部列入
         * NON_NULL:不显示空字段（对象未初始化）
         * NON_DEFAULT：不显示与默认值相同的字段
         * NON_EMPTY：不显示仅含空格的字符串（对象初始化，但为空）
         */
        objectMapper.setSerializationInclusion(Inclusion.ALWAYS);

        /**
         * 取消默认转换timestamps形式
         * true：返回从1970年1月1号到某个时间点的毫秒数
         */
        objectMapper.configure(SerializationConfig.Feature.WRITE_DATES_AS_TIMESTAMPS,false);

        //忽略空Bean（无get/set方法）转json的错误
        objectMapper.configure(SerializationConfig.Feature.FAIL_ON_EMPTY_BEANS,false);

        //所有的日期格式都统一为以下的样式，即yyyy-MM-dd HH:mm:ss
        objectMapper.setDateFormat(new SimpleDateFormat(DateTimeUtil.STANDARD_FORMAT));

        //忽略 在json字符串中存在，但是在java对象中不存在对应属性的情况（如：字符串中新增A字段，Java对象中没有）。防止错误
...
//<T>声明为泛型方法，T2为返回值类型，T3为限制class类型
